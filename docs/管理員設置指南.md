...
| 頁面 | URL |
|------|-----|
| 管理員登入 | `http://localhost:5173/admin/login` |
| 管理後台 Dashboard | `http://localhost:5173/admin` |
...
curl -X POST http://localhost:8000/api/auth/admin-login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@yourdomain.com","password":"Admin@Pass123"}'
```
...
| 信任分數變更日誌 | `GET /api/admin/users/{user_id}/trust-logs` |
...
| 審核申訴 | `POST /api/moderation/appeals/{appeal_id}/review` |
| 待審核照片列表 | `GET /api/admin/photos/pending` |
| 照片審核統計 | `GET /api/admin/photos/stats` |
| 照片詳情 | `GET /api/admin/photos/{photo_id}` |
| 審核照片 | `POST /api/admin/photos/{photo_id}/review` |
| 隔離照片預覽 | `GET /api/admin/photos/quarantine/{user_id}/{filename}` |
