...
- [x] TrustScoreService 服務
- [x] 自動分數調整機制
- [x] 配對算法整合（5 分權重）
- [x] 低信任用戶訊息限制（< 20 分每日 20 則）
- [x] 22 個單元測試
- [x] **分數規則：**
  - 正向：Email 驗證 +5、被喜歡 +1、配對成功 +2
  - 負向：被舉報 -5、舉報確認 -10、違規內容 -3、被封鎖 -2
  - 範圍：0-100（預設 50，閾值 20）
...
```
✅ 已完成:
  - 11 個 API 模組（含通知、內容審核、照片審核）
  - 9 個資料模型（含 Notification）
  - 8 個服務層（含 TrustScoreService）
  - 49+ 個 API 端點
  - 17 個前端頁面
  - 16 個 Vue 組件
  - 7 個 Pinia Stores
...
- [README.md](../README.md) - 專案總覽（含快速開始）
