...
| ID | 問題 | 工作量 | 狀態 |
| --- | --- | --- | --- |
| ~~P1-2~~ | ~~照片審核機制~~ | 4-8 小時 | ✅ 已完成 |
| P1-3 | 每日喜歡次數限制 | 2 小時 | ❌ 待實作 |
| ~~P1-4~~ | ~~圖片/GIF 訊息~~ | 4-6 小時 | ✅ 已完成 |
| P1-5 | WebSocket 多實例支援 | 1 週 | ❌ 待實作 |
| P1-6 | Match 表查詢效率 | 4 小時 | ❌ 待實作 |
| P1-7 | 監控與日誌系統 | 3-4 小時 | ❌ 待實作 |

**已完成 P1 項目**：

- ✅ P1-2: 照片審核機制（services/photo_moderation.py + api/photo_moderation.py）
...
| 項目 | 狀態 | 說明 |
| --- | --- | --- |
| Redis 整合 | 🔄 部分使用 | 登入限制、Token 黑名單已使用，快取/Session 待實作 |
| Token 黑名單 | ✅ 已用 Redis | 優先 Redis，自動 fallback 內存 |
