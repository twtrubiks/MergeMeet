...
**特色**：多維度配對推薦 ｜ 信任分數機制 ｜ 即時聊天 ｜ Claude Code 整合開發
...
| **版本** | 1.0.0 MVP |
| **開發進度** | ✅ Week 1-6 已完成 |
| **代碼規模** | 純程式碼約 37,267 行（後端 7,495 + 前端 18,613 + 測試 11,159），含空行和註釋共 53,279 行 |
...
![](./docs/screenshots/07-likes-you.png)
...
| **探索配對** | Tinder 風格卡片、按鈕與鍵盤快捷鍵操作、5 維度配對演算法、誰喜歡我、撤銷跳過、空池一鍵放寬 |
...
5 維度加權評分配對系統，綜合評估興趣匹配（50%）、地理距離（20%）、活躍度（20%）、檔案完整度（5%）和信任分數（5%），為用戶推薦最適合的對象。

MVP 階段採用可解釋的規則式評分（rule-based scoring）——權重透明、可調參、可單元測試，避免無行為資料時導入 ML 的冷啟動問題；待累積足夠互動資料後，可演進為學習式排序。
