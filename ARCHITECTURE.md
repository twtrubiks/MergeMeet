...
| P0-2 | 推播通知缺失 | - | 1-2 天 | 留存率極低 |
| P0-3 | Email 服務未整合 | auth.py | 2-4 小時 | 假帳號氾濫 |
| P0-4 | 登入失敗無限制 | auth.py | 1 小時 | 暴力破解風險 |
| P0-5 | 全局速率限制缺失 | main.py | 1 小時 | DoS 攻擊風險 |
| P0-6 | HTTPS/SSL 未配置 | - | 1-2 小時 | 安全風險 |
| P0-7 | 資料庫備份缺失 | - | 2-3 小時 | 資料遺失風險 |
| P0-8 | WebSocket Token 不安全 | websocket | 30 分鐘 | Token 洩漏 |

### 16.2 P1 - 高優先級（本週內處理）

| ID | 問題 | 工作量 |
|----|------|--------|
| P1-1 | 地理位置自動定位 | 1-2 小時 |
| P1-2 | 照片審核機制 | 4-8 小時 |
| P1-3 | 每日喜歡次數限制 | 2 小時 |
...
| 項目 | 狀態 | 說明 |
|------|------|------|
| Redis 整合 | 🔄 未完全使用 | 目前僅配置，快取/Session 未實作 |
