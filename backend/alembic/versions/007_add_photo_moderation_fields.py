...
def upgrade() -> None:
    ...
    op.add_column(
        "photos",
        sa.Column(
            "moderation_status", sa.String(length=20), nullable=False, server_default="APPROVED"
        ),
    )

    # 新增拒絕原因欄位
    op.add_column("photos", sa.Column("rejection_reason", sa.Text(), nullable=True))

    # 新增審核者欄位
    op.add_column("photos", sa.Column("reviewed_by", postgresql.UUID(as_uuid=True), nullable=True))

    # 新增審核時間欄位
    op.add_column("photos", sa.Column("reviewed_at", sa.DateTime(timezone=True), nullable=True))

    # 新增自動審核分數欄位（預留擴展）
    op.add_column("photos", sa.Column("auto_moderation_score", sa.Integer(), nullable=True))

    # 新增自動審核標籤欄位（預留擴展）
    op.add_column("photos", sa.Column("auto_moderation_labels", sa.Text(), nullable=True))
    ...
    op.create_foreign_key(
        "fk_photos_reviewed_by", "photos", "users", ["reviewed_by"], ["id"], ondelete="SET NULL"
    )
