...
async def create_profile_with_photos(
    client: AsyncClient, headers: dict, mock_file_storage, num_photos: int = 3
) -> list[str]:
    ...
    """
    # 建立 Profile
    profile_data = {"display_name": "測試用戶", "gender": "male", "bio": "測試個人簡介"}
...
class TestReorderPhotos:
    ...
    async def test_reorder_photos_success(
        ...
        resp = await client.put(
            "/api/profile/photos/order", json={"photo_ids": new_order}, headers=auth_headers
        )
    ...
    async def test_reorder_photos_invalid_count(
        ...
        # 只傳 2 張照片 ID
        resp = await client.put(
            "/api/profile/photos/order", json={"photo_ids": photo_ids[:2]}, headers=auth_headers
        )
    ...
    async def test_reorder_photos_unauthorized(self, client: AsyncClient):
        """錯誤：未登入"""
        resp = await client.put(
            "/api/profile/photos/order", json={"photo_ids": [str(uuid.uuid4())]}
        )

        assert resp.status_code == 401

    @pytest.mark.asyncio
    async def test_reorder_photos_no_profile(self, client: AsyncClient, auth_headers: dict):
        """錯誤：無 Profile"""
        resp = await client.put(
            "/api/profile/photos/order",
    ...
    async def test_reorder_single_photo(
        ...
        resp = await client.put(
            "/api/profile/photos/order", json={"photo_ids": photo_ids}, headers=auth_headers
        )
...
async def approve_photos(test_db: AsyncSession, photo_ids: list[str]):
    """將指定照片設為已審核通過"""
...
class TestSetProfilePicture:
    ...
    async def test_set_profile_picture_reorders(
        ...
        # 將第三張（index=2）設為主頭像
        await client.put(f"/api/profile/photos/{photo_ids[2]}/primary", headers=auth_headers)
    ...
    async def test_set_profile_picture_invalid_id(
        ...
        """錯誤：照片 ID 不存在"""
        # 建立 Profile
        await create_profile_with_photos(client, auth_headers, mock_file_storage, num_photos=1)

        # 使用不存在的照片 ID
        fake_id = str(uuid.uuid4())
        resp = await client.put(f"/api/profile/photos/{fake_id}/primary", headers=auth_headers)

        assert resp.status_code == 404
        assert "不存在或不屬於此用戶" in resp.json()["detail"]

    @pytest.mark.asyncio
    async def test_set_profile_picture_unauthorized(self, client: AsyncClient):
        """錯誤：未登入"""
        fake_id = str(uuid.uuid4())
        resp = await client.put(f"/api/profile/photos/{fake_id}/primary")

        assert resp.status_code == 401

    @pytest.mark.asyncio
    async def test_set_profile_picture_no_profile(self, client: AsyncClient, auth_headers: dict):
        """錯誤：無 Profile"""
        fake_id = str(uuid.uuid4())
        resp = await client.put(f"/api/profile/photos/{fake_id}/primary", headers=auth_headers)
    ...
    async def test_set_profile_picture_already_primary(
        ...
        resp = await client.put(f"/api/profile/photos/{photo_ids[0]}/primary", headers=auth_headers)
