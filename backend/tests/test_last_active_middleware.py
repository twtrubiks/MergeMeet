...
class TestLastActiveMiddleware:
    ...
    async def test_last_active_updated_on_authenticated_request(
        self, client: AsyncClient, auth_user: dict, test_db: AsyncSession
    ):
        """測試已認證請求成功後會更新 last_active"""
        # 取得用戶 ID
        result = await test_db.execute(select(User.id).where(User.email == auth_user["email"]))
        ...
        response = await client.post(
            ...
            json={
                ...
                "gender": "male",
                "bio": "Test bio",
                "location": {"latitude": 25.0330, "longitude": 121.5654, "location_name": "Taipei"},
            },
        )
    ...
    @pytest.mark.asyncio
    async def test_last_active_not_updated_on_unauthenticated_request(self, client: AsyncClient):
        """測試未認證請求不會觸發更新（也不會報錯）"""
        # 發送未認證請求
    ...
    async def test_last_active_not_updated_on_failed_request(
        ...
        # 發送會失敗的請求（不存在的端點）
        response = await client.get("/api/nonexistent", headers=auth_user["headers"])
    ...
    @pytest.mark.asyncio
    async def test_middleware_handles_invalid_token_gracefully(self, client: AsyncClient):
        """測試無效 Token 時 Middleware 靜默處理"""
        # 發送帶有無效 Token 的請求
        response = await client.get("/health", headers={"Authorization": "Bearer invalid_token"})
        # 請求應該成功（health 不需要認證）
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_middleware_handles_expired_token_gracefully(self, client: AsyncClient):
        ...
        response = await client.get("/health", headers={"Authorization": f"Bearer {expired_token}"})
        # 請求應該成功（health 不需要認證）
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_last_active_updated_with_cookie_auth(
        self, client: AsyncClient, sample_user_data: dict, test_db: AsyncSession
    ):
