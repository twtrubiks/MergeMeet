...
async def test_verify_email_success(client: AsyncClient):
    ...
    # 驗證 Email
    response = await client.post(
        "/api/auth/verify-email", json={"email": "verify@example.com", "verification_code": code}
    )
...
async def test_verify_email_wrong_code(client: AsyncClient):
    ...
    response = await client.post(
        ...
        json={"email": "wrongcode@example.com", "verification_code": "000000"},
    )
...
async def test_resend_verification_invalid_email_format(client: AsyncClient):
    ...
    response = await client.post(
        "/api/auth/resend-verification", json={"email": "not-a-valid-email"}
    )
...
async def test_verify_email_nonexistent_user_no_leak(client: AsyncClient):
    ...
    response = await client.post(
        "/api/auth/verify-email",
        json={"email": "nonexistent_verify_12345@example.com", "verification_code": "123456"},
    )
...
@pytest.mark.asyncio
async def test_change_password_token_invalidated(
    client: AsyncClient, user_for_password_change: dict
):
    """測試修改密碼後原 Token 失效"""
    token = user_for_password_change["token"]
    csrf_token = user_for_password_change["csrf_token"]
...
async def test_change_password_no_digit(client: AsyncClient, user_for_password_change: dict):
    ...
    response = await client.post(
        "/api/auth/change-password",
        headers={"Authorization": f"Bearer {token}", "X-CSRF-Token": csrf_token},
        json={"current_password": "OldPassword123", "new_password": "NewPasswordOnly"},
    )
...
@pytest.mark.asyncio
async def test_verify_reset_token_returns_masked_email(
    client: AsyncClient, db_session: AsyncSession
):
    ...
    # 驗證 token
    response = await client.get("/api/auth/verify-reset-token", params={"token": reset_token})
...
async def test_verify_reset_token_invalid(client: AsyncClient):
    ...
    response = await client.get(
        "/api/auth/verify-reset-token", params={"token": "invalid_token_here"}
    )
...
async def test_verify_email_rate_limit(client: AsyncClient):
    ...
    await client.post(
        "/api/auth/register",
        json={"email": unique_email, "password": "Password123", "date_of_birth": "1995-01-01"},
    )

    # 嘗試 5 次錯誤驗證碼
    for i in range(5):
        response = await client.post(
            "/api/auth/verify-email", json={"email": unique_email, "verification_code": "000000"}
        )
        if i < 4:
    ...
    # 被鎖定後再次嘗試應該返回 429
    response = await client.post(
        "/api/auth/verify-email", json={"email": unique_email, "verification_code": "123456"}
    )
...
async def test_reset_password_invalidates_tokens(client: AsyncClient, db_session: AsyncSession):
    ...
    reset_response = await client.post(
        "/api/auth/reset-password", json={"token": reset_token, "new_password": "NewPassword456"}
    )
