# MergeMeet 管理員設置指南

## 📋 目錄
1. [管理員帳號創建](#管理員帳號創建)
2. [密碼設置流程](#密碼設置流程)
3. [管理員權限說明](#管理員權限說明)
4. [安全最佳實踐](#安全最佳實踐)
5. [常見問題](#常見問題)

---

## 管理員帳號創建

### 方法一：透過資料庫直接創建（推薦用於初始化）

```bash
# 1. 進入 backend 目錄
cd backend

# 2. 使用 Python 生成密碼哈希
python3 -c "
import sys
sys.path.insert(0, '.')
from app.core.security import get_password_hash

password = 'your_secure_password_here'  # 替換為你的強密碼
hashed = get_password_hash(password)
print(f'Password hash: {hashed}')
"

# 3. 將哈希密碼插入資料庫
docker exec mergemeet_postgres psql -U mergemeet -d mergemeet -c "
INSERT INTO users (id, email, password_hash, date_of_birth, is_admin, email_verified, is_active)
VALUES (
    gen_random_uuid(),
    'admin@yourdomain.com',  -- 替換為管理員 email
    '\$2b\$12\$xxxxx...',     -- 替換為上面生成的哈希
    '1990-01-01',            -- 管理員生日（18+ 驗證）
    true,                    -- 設置為管理員
    true,                    -- Email 已驗證
    true                     -- 帳號啟用
);
"
```

### 方法二：透過註冊 API + 資料庫更新

```bash
# 1. 使用一般註冊流程創建帳號
curl -X POST http://localhost:8000/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{
    "email": "admin@yourdomain.com",
    "password": "SecurePassword123!",
    "date_of_birth": "1990-01-01"
  }'

# 2. 將該帳號提升為管理員
docker exec mergemeet_postgres psql -U mergemeet -d mergemeet -c "
UPDATE users
SET is_admin = true, email_verified = true
WHERE email = 'admin@yourdomain.com';
"
```

---

## 密碼設置流程

### 密碼要求
- **最小長度**: 8 字元
- **建議長度**: 12+ 字元
- **複雜度**: 包含大小寫字母、數字、特殊字元
- **禁止**: 常見密碼、個人資訊、連續字元

### 密碼哈希機制
MergeMeet 使用雙重哈希保護密碼：
1. **SHA256 預處理**: 避免 bcrypt 72 bytes 限制
2. **bcrypt 加密**: 防止彩虹表攻擊

```python
# 密碼處理流程（app/core/security.py）
def get_password_hash(password: str) -> str:
    # 1. SHA256 預處理
    pre_hashed = hashlib.sha256(password.encode('utf-8')).hexdigest()
    # 2. bcrypt 加密
    return pwd_context.hash(pre_hashed)
```

### 重置管理員密碼

**步驟 1: 生成新密碼哈希**
```bash
cd backend
python3 -c "
import sys
sys.path.insert(0, '.')
from app.core.security import get_password_hash

new_password = 'NewSecurePassword123!'
print('New password hash:', get_password_hash(new_password))
"
```

**步驟 2: 更新資料庫**
```bash
docker exec mergemeet_postgres psql -U mergemeet -d mergemeet -c "
UPDATE users
SET password_hash = '\$2b\$12\$xxxxx...'  -- 替換為新的哈希
WHERE email = 'admin@mergemeet.com';
"
```

**步驟 3: 驗證登入**
```bash
curl -X POST http://localhost:8000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "admin@mergemeet.com",
    "password": "NewSecurePassword123!"
  }'
```

---

## 管理員權限說明

### 可執行操作
✅ **內容審核**
- 查看所有舉報
- 審核舉報並採取行動
- 管理申訴

✅ **用戶管理**
- 封禁/解禁用戶
- 設置封禁期限
- 查看用戶詳情

✅ **敏感詞管理**
- 新增/編輯/刪除敏感詞
- 設置過濾級別（警告/拒絕/自動封禁）
- 配置正則表達式規則

✅ **數據統計**
- 系統使用統計
- 用戶活躍度分析
- 舉報趨勢監控

### 受限操作
❌ **不可執行**
- 無法封禁其他管理員
- 無法直接查看用戶私密訊息（需透過舉報機制）
- 無法繞過系統審計日誌

---

## 安全最佳實踐

### 1. 密碼管理
```
✅ 使用密碼管理器生成強密碼
✅ 定期更換密碼（建議 90 天）
✅ 不同系統使用不同密碼
✅ 啟用雙因素認證（如已實作）
❌ 不在代碼或配置文件中硬編碼密碼
❌ 不透過不安全通道分享密碼
```

### 2. 帳號保護
```bash
# 檢查管理員帳號
docker exec mergemeet_postgres psql -U mergemeet -d mergemeet -c "
SELECT email, is_admin, is_active, created_at, updated_at
FROM users
WHERE is_admin = true;
"

# 停用閒置管理員帳號
docker exec mergemeet_postgres psql -U mergemeet -d mergemeet -c "
UPDATE users
SET is_active = false
WHERE email = 'old.admin@example.com';
"
```

### 3. 審計日誌
```bash
# 查看管理員操作記錄（需實作審計日誌功能）
# TODO: 實作管理員操作審計日誌
```

### 4. IP 白名單（生產環境建議）
```python
# config.py - 限制管理後台訪問 IP
ADMIN_ALLOWED_IPS = [
    "192.168.1.0/24",  # 內網
    "1.2.3.4"          # 特定 IP
]
```

---

## 常見問題

### Q1: 忘記管理員密碼怎麼辦？
**A:** 按照[重置管理員密碼](#重置管理員密碼)流程操作。

### Q2: 如何查看目前有哪些管理員？
```bash
docker exec mergemeet_postgres psql -U mergemeet -d mergemeet -c "
SELECT email, is_admin, is_active, created_at
FROM users
WHERE is_admin = true;
"
```

### Q3: 可以有多個管理員嗎？
**A:** 可以，但建議：
- 生產環境：2-3 個管理員帳號
- 每個管理員使用獨立帳號（可追蹤操作）
- 定期審查管理員名單

### Q4: 如何撤銷管理員權限？
```bash
docker exec mergemeet_postgres psql -U mergemeet -d mergemeet -c "
UPDATE users
SET is_admin = false
WHERE email = 'former.admin@example.com';
"
```

### Q5: 管理員密碼有特殊要求嗎？
**A:**
- 最小長度：8 字元
- **強烈建議**：12+ 字元 + 大小寫字母 + 數字 + 特殊字元
- **禁止**：password、admin、123456 等常見密碼

### Q6: 測試環境的預設管理員帳號？
```
Email: admin@mergemeet.com
密碼: admin123 (測試用，生產環境務必更改！)
```

---

## 快速參考

### 創建管理員（完整流程）
```bash
# 1. 生成密碼哈希
cd /home/twtrubiks/下載/merge_meet/mergemeet/backend
HASH=$(python3 -c "
import sys
sys.path.insert(0, '.')
from app.core.security import get_password_hash
print(get_password_hash('YourSecurePassword123!'))
")

# 2. 插入資料庫
docker exec mergemeet_postgres psql -U mergemeet -d mergemeet <<EOF
INSERT INTO users (id, email, password_hash, date_of_birth, is_admin, email_verified, is_active)
VALUES (
    gen_random_uuid(),
    'admin@yourdomain.com',
    '$HASH',
    '1990-01-01',
    true,
    true,
    true
);
EOF

# 3. 驗證登入
curl -X POST http://localhost:8000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@yourdomain.com","password":"YourSecurePassword123!"}'
```

### 測試登入
```bash
# Web UI
http://localhost:5173/admin/login

# API 測試
curl -X POST http://localhost:8000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@mergemeet.com","password":"admin123"}'
```

---

## 相關文檔
- [API 文檔](http://localhost:8000/docs) - FastAPI Swagger UI
- [資料庫Schema](/home/twtrubiks/下載/merge_meet/mergemeet/backend/app/models/user.py)
- [安全配置](/home/twtrubiks/下載/merge_meet/mergemeet/backend/app/core/security.py)

---

**文檔版本**: v1.0
**最後更新**: 2025-12-13
**維護者**: MergeMeet Development Team
